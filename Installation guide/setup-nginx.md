...
*Replace `ztcontroller.ghtklab.com` with your actual domain name.*

```bash
sudo cp ztcontroller.ghtklab.com.crt /etc/ssl/certs/
sudo cp ztcontroller.ghtklab.com.key /etc/ssl/private/
sudo chmod 640 /etc/ssl/private/ztcontroller.ghtklab.com.key
sudo chown root:nginx /etc/ssl/private/ztcontroller.ghtklab.com.key
```
...
```bash
sudo nano /etc/nginx/sites-available/ztcontroller.ghtklab.com.conf
```

Paste the following configuration. **Note:** This configuration handles Public API, Internal API, and Headscale traffic.

<details>
<summary><strong>Click to view `ztcontroller.ghtklab.com.conf`</strong></summary>
...
```bash
sudo ln -s /etc/nginx/sites-available/ztcontroller.ghtklab.com.conf /etc/nginx/sites-enabled/
sudo nginx -t
```
...
openssl s_client -connect ztcontroller.ghtklab.com:443 -servername ztcontroller.ghtklab.com </dev/null 2>/dev/null | openssl x509 -noout -subject -issuer -dates

# Check API Routes
curl -I https://ztcontroller.ghtklab.com/public/healthz
curl -X POST https://internal-ztcontroller.ghtklab.com/internal/api/v1/auth/login -k -H "Content-Type: application/json" -d '{"username":"admin","password":"admin"}'  # Should work from allowed IPs
curl -I https://ztcontroller.ghtklab.com/                          # Headscale check
```
...
Create a configuration file for the frontend (e.g., `/etc/nginx/sites-available/ztrust.ghtklab.com.conf`).

<details>
<summary><strong>Click to view `ztrust.ghtklab.com.conf`</strong></summary>

```nginx
server {
    listen 443 ssl;
    server_name ztrust.ghtklab.com; # Replace with your domain

    ssl_certificate     /etc/ssl/certs/ztrust.ghtklab.com.crt; # Path to your certificate
    ssl_certificate_key /etc/ssl/private/ztrust.ghtklab.com.key; # Path to your key
    ssl_protocols TLSv1.2 TLSv1.3;
    ssl_ciphers HIGH:!aNULL:!MD5;
...
```bash
# Enable the site
sudo ln -s /etc/nginx/sites-available/ztrust.ghtklab.com.conf /etc/nginx/sites-enabled/
